// Flight Review (default) page — entry view. Renders the PX4 standard plot set
// from a ULog parsed entirely in the browser.

PawaStore.initTheme();

let model = null;   // parsed ULog model for the loaded file
...
function baseLayout(extra = {}) {
  ...
  return Object.assign({
    ...
    xaxis: {
      gridcolor: t.grid, zerolinecolor: t.zero, linecolor: t.axis, tickcolor: t.tick,
      title: { text: "time [s]", font: { color: t.textDim, size: 10 } },
    },
    yaxis: { gridcolor: t.grid, zerolinecolor: t.zero, linecolor: t.axis, tickcolor: t.tick },
    legend: {
      bgcolor: t.legendBg, bordercolor: t.legendBord, borderwidth: 1,
      font: { size: 10 }, orientation: "v", x: 1.005, xanchor: "left", y: 0.88, yanchor: "top",
    },
    hovermode: "x unified",
...
function renderTimePanel(target, panel) {
  const traces = panel.series.map(s => ({
    x: s.t, y: s.y, name: s.name, type: "scatter", mode: "lines",
...
function renderScatterXY(target, panel) {
  const traces = panel.series.map(s => {
    ...
    const trace = {
      x: s.x, y: s.y, name: s.name, type: "scatter", mode,
      line: { color, width: 1.5 },
      hovertemplate: `<b>${s.name}</b><br>E=%{x:.2f} m<br>N=%{y:.2f} m<extra></extra>`,
    };
    if (mode === "markers") trace.marker = { size: 9, color: "rgba(0,0,0,0)", symbol: "circle", line: { color, width: 1.6 } };
    else trace.marker = { size: 3, color };
    return trace;
  });
  const layout = baseLayout({
...
function buildPanelDOM(idx, panel) {
  ...
  if (panel.type === "map") plotDiv.className = "review-map";
  else plotDiv.className = "review-plot" + (panel.type === "scatter_xy" ? " tall" : "");
...
function buildInfoCard(info) {
  ...
  for (const colRows of [rows.slice(0, half), rows.slice(half)]) {
...
function renderMap(target, panel) {
  ...
  const line = L.polyline(coords, { color: s.color || "#a78bfa", weight: 3, opacity: 0.95 }).addTo(map);
...
async function renderPanels() {
  ...
  setStatus("Building plot data…");
  await new Promise((r) => requestAnimationFrame(r));

  let res;
  try {
    res = PawaAnalysis.getDefaultPlotData(model);
  } catch (e) {
    els.loading.hidden = true; els.welcome.hidden = false;
...
async function renderPanelsFromData(res) {
  ...
  // File-info header card (above panel 01).
  try {
    const info = PawaAnalysis.getFileInfo(model);
    if (info && info.ok) els.grid.appendChild(buildInfoCard(info));
  } catch (_) { /* non-fatal */ }
  ...
  for (let i = 0; i < res.panels.length; i++) {
    ...
    try {
      if (p.type === "map") renderMap(plotDiv, p);
      else if (p.type === "scatter_xy") renderScatterXY(plotDiv, p);
      else renderTimePanel(plotDiv, p);
    } catch (e) {
...
els.customBtn.addEventListener("click", () => { window.location.href = "inspector.html"; });
...
// ----- Theme toggle (update plots in place, no re-render) -----
function applyThemeToPlots() {
  ...
  document.querySelectorAll(".review-plot").forEach(div => {
    ...
    const update = {
      ...
      "font.color": t.text,
      "legend.bgcolor": t.legendBg, "legend.bordercolor": t.legendBord,
      "hoverlabel.bgcolor": t.hoverBg, "hoverlabel.bordercolor": t.hoverBord, "hoverlabel.font.color": t.text,
    };
